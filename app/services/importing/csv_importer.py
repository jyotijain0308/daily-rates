...
from app.services.country_service import ensure_country
...
class ProductCSVImporter:
    ...
    @staticmethod
    def normalize_position_row(values: List[str]) -> Dict:
        """Normalize a CSV row by expected column order."""
        padded_values = list(values) + [''] * (len(ProductCSVImporter.CSV_COLUMNS) - len(values))
        return {
            'serial_no': padded_values[0],
            'country_of_origin': padded_values[1],
            'shipment_by': padded_values[2],
            'product_name': padded_values[3],
            'weight_kg': padded_values[4],
            'packing': padded_values[5],
            'price_aed': padded_values[6],
        }
    
    @staticmethod
    def validate_row(row: Dict, row_number: int) -> Tuple[bool, Dict]:
        """
        ...
        for field, label in [
            ...
            if field not in row or row[field] in (None, ''):
                return False, ProductCSVImporter._validation_error(
                    row_number,
                    f"Missing required column '{label}'",
                )
        
        if not row['product_name'].strip():
            return False, ProductCSVImporter._validation_error(row_number, "'Product Name' cannot be empty")
        
        if len(row['product_name']) > 255:
            return False, ProductCSVImporter._validation_error(
                row_number,
                "'Product Name' exceeds 255 characters",
            )
        
        if len(row['country_of_origin']) > 100:
            return False, ProductCSVImporter._validation_error(
                row_number,
                "'Country of origin' exceeds 100 characters",
            )

        if len(row['shipment_by']) > 100:
            return False, ProductCSVImporter._validation_error(
                row_number,
                "'Shipment by' exceeds 100 characters",
            )
        ...
        try:
            ...
            return False, ProductCSVImporter._validation_error(
                row_number,
                "'S.No.' must be a whole number",
                row['serial_no'],
            )

        if not row['weight_kg'].strip():
            return False, ProductCSVImporter._validation_error(row_number, "'Weight in kg' cannot be empty")
        ...
        try:
            ...
            return False, ProductCSVImporter._validation_error(
                row_number,
                "'Price in AED' must be a valid number",
                row['price_aed'],
            )
    ...
    @staticmethod
    def _validation_error(row_number: int, message: str, value=None) -> Dict:
        return {
            'row': row_number,
            'message': message,
    ...
    def _format_validation_error(error: Dict) -> str:
        suffix = f", got '{error['value']}'" if error.get('value') is not None else ""
        return f"Row {error['row']}: {error['message']}{suffix}"

    @staticmethod
    def _format_validation_errors(errors: List[Dict]) -> List[str]:
        grouped = OrderedDict()
        for error in errors:
            key = (error.get('message'), error.get('value'))
            grouped.setdefault(key, []).append(error)

        formatted = []
        for (_message, _value), group in grouped.items():
            if len(group) == 1:
                formatted.append(ProductCSVImporter._format_validation_error(group[0]))
                continue

            rows = ", ".join(str(error['row']) for error in group)
            suffix = f", got '{group[0]['value']}'" if group[0].get('value') is not None else ""
            formatted.append(f"Rows {rows}: {group[0]['message']}{suffix}")
    ...
    def parse_csv_content(file_content: str) -> Tuple[List[Dict], List[str]]:
        ...
        try:
            ...
            header = rows[0]
            if len(header) < len(ProductCSVImporter.CSV_COLUMNS):
                errors.append(
                    "CSV must have at least 7 columns in this order: "
                    f"{', '.join(ProductCSVImporter.CSV_COLUMNS)}"
                )
                return [], errors
            ...
            for row_num, row in enumerate(rows[1:], start=2):
                ...
                normalized_row = ProductCSVImporter.normalize_position_row(row)
                is_valid, error = ProductCSVImporter.validate_row(normalized_row, row_num)
            ...
            errors.extend(ProductCSVImporter._format_validation_errors(validation_errors))
    ...
    @staticmethod
    def _import_price_deduction_percent(company_id) -> float:
        settings = company_settings_for(company_id)
        return float(getattr(settings, 'import_price_deduction_percent', 15.0) or 0)

    @staticmethod
    def _apply_import_price_deduction(price, deduction_percent: float) -> float:
        price_decimal = Decimal(str(price))
        multiplier = Decimal('1') - (Decimal(str(deduction_percent)) / Decimal('100'))
        return float((price_decimal * multiplier).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))

    @staticmethod
    def build_import_plan(rows: List[Dict], company_id=None) -> Dict:
        """Build create/update/skip decisions for valid CSV rows."""
        company_id = company_id or current_company_id()
        deduction_percent = ProductCSVImporter._import_price_deduction_percent(company_id)
        ...
        for row in rows:
            ...
            if key in seen_keys:
                ...
                plan_rows.append({
                    **row,
                    ...
                    'old_price_aed': None,
                    'new_price_aed': ProductCSVImporter._apply_import_price_deduction(
                        row['price_aed'],
                        deduction_percent,
                    ),
                    'uploaded_price_aed': float(row['price_aed']),
                    'import_price_deduction_percent': deduction_percent,
                    'change_amount': None,
                    'change_percent': None,
                    'large_change': False,
                })
            ...
            uploaded_price = float(row['price_aed'])
            new_price = ProductCSVImporter._apply_import_price_deduction(
                uploaded_price,
                deduction_percent,
            )
            ...
            if not product:
                ...
                plan_rows.append({
                    **row,
                    ...
                    'uploaded_price_aed': uploaded_price,
                    'import_price_deduction_percent': deduction_percent,
                    **diff,
                })
    ...
    @staticmethod
    def import_products(rows: List[Dict], changed_by: str = 'import', company_id=None) -> Tuple[Dict, List[str]]:
        """
        ...
        try:
            company_id = company_id or current_company_id()
            plan = ProductCSVImporter.build_import_plan(rows, company_id=company_id)
            ...
            for plan_row in plan['rows']:
                ...
                try:
                    ...
                    new_price = float(plan_row['new_price_aed'])
                    ...
                    if product:
                        ...
                        product.weight_kg = plan_row['weight_kg'].strip()
                        ...
                        product = Product(
                            ...
                            product_name=product_name,
                            weight_kg=plan_row['weight_kg'].strip(),
    ...
    @staticmethod
    def import_from_file(file_path: str, company_id=None) -> Tuple[int, List[str], List[Dict]]:
        """
        ...
        # Import valid rows
        import_summary, import_errors = ProductCSVImporter.import_products(valid_rows, company_id=company_id)
